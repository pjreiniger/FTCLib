...
public class MotorGroup extends Motor implements Iterable<Motor> {
    ...
    public List<Double> getSpeeds() {
        return Arrays.stream(group).map(Motor::get).collect(Collectors.toList());
    }
    ...
    /**
     * @return All current velocities of the motors in the group in units of distance per second which
     *     is by default ticks / second
     */
    public List<Double> getVelocities() {
        return Arrays.stream(group).map(Motor::getRate).collect(Collectors.toList());
    }
    ...
    /**
     * @return The position of every motor in the group in units of distance which is by default ticks
     */
    public List<Double> getPositions() {
        return Arrays.stream(group).map(Motor::getDistance).collect(Collectors.toList());
    }
